...
var downloadExportedFile = function downloadExportedFile(e) {
  ...
  var url = urlTemplate.replace('{id}', content.id).replace('{name}', content.name).replace('{type}', content.type).replace('{file}', content.file);
  return fetch(url, {
    method: 'GET',
...
var layouts = [{
  ...
  onSelected: function onSelected(viewer) {
    ...
    var primaryPublicationSelector = document.querySelector('#primary-publication-selector');
    var comparePublicationSelector = document.querySelector('#compare-publication-selector');
    viewer.comparePublications(primaryPublicationSelector.value || window.publications[0].id, comparePublicationSelector.value || window.publications[0].id, 'sideBySide');
  ...
  onSelected: function onSelected(viewer) {
    ...
    var primaryPublicationSelector = document.querySelector('#primary-publication-selector');
    var comparePublicationSelector = document.querySelector('#compare-publication-selector');
    viewer.comparePublications(primaryPublicationSelector.value || window.publications[0].id, comparePublicationSelector.value || window.publications[0].id, 'text');
    enablePublicationSelector(viewer, window.publications, true);
  }
}];
var currentLayout = '';
