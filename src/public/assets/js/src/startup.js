const shouldRefresh = () => {
  ...
  const EXPIRATION_WINDOW_IN_SECONDS = window.accessTokenExpires
    ? window.accessTokenExpires / 2
    : 300
  const expirationWindowStart =
...
export const startup = (viewerAuthority) => {
  refreshToken()
    .then(() => {
      return fetch(`${viewerAuthority}/viewer/api/v1/viewers/brava-view-1.x/loader`, {
        headers: {
          authorization: window.accessToken
        }
      })
